...
def loaders(csv_path, ratio_labelled = 0.80, ratio_unlabelled = 0.20):
    ...
    # Combinamos todos en un solo dict para classDict
    classDict = {}
...
def forward_head(y_pred, y, heads):
    ...
    for super_idx, head in enumerate(heads):
        ...
        feats = y_pred[batch_idxs]       # (b_i, F)
        local_logits = head(feats)       # (b_i, C_local)
    ...
    for epoch in range(num_epochs + warmup_epochs):
        ...
        if epoch < warmup_epochs:
            ...
            for batch_id, ((x_lab, y_super_lab, y_lab), (x_unlab, y_super_unlab)) in enumerate(zip(labelled_loader, unlabelled_loader)):
                ...
                # ---- Forward labelled ----
                y_super_pred_lab = model(x_lab)
                y_sub_pred_lab = forward_head(y_super_pred_lab, y_super_lab, heads)
                ...
                # ---- Forward unlabelled ----
                y_super_pred_unlab = model(x_unlab)
                y_sub_pred_unlab = forward_head(y_super_pred_unlab, y_super_unlab, heads)

                x_unlab_aug = torch.stack([randAugment(T.ToPILImage()(img.cpu())) for img in x_unlab]).to(device)
                y_super_pred_aug = model.forward(x_unlab_aug)
                y_sub_pred_aug = forward_head(y_super_pred_aug, y_super_unlab, heads)
...
def eval(model, val_loader, heads):
    ...
    with torch.no_grad():
        for x_lab, y_super_lab, y_lab in val_loader:
            ...
            # --- forward ---
            y_super_pred = model(x_lab)
            y_sub_pred = forward_head(y_super_pred, y_super_pred.argmax(dim=1), heads)
...
head = ClassifierHead(num_classes=num_classes, input_size=projection_size)

model = nn.Sequential(backbone, projection_layer, head).to(device)

general_loader, labelled_loader, unlabelled_loader, classDict = loaders(r"unlabelled_train_20pct.csv")
...
for idx, label in enumerate(list(classDict.values())):
    if prev != label // 1000:
        if prev != 0:
            # TODO:
            heads.append(ClassifierHeadSmall(count, input_size = 11))
